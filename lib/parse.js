...
/** Human readable status letters for porcelain v2 XY codes. */
const XY_MEANING = {
...
/** Strip ANSI escape sequences defensively (git may emit them despite config). */
...
/**
 * Human display name for one porcelain status letter.
 * @param {string} letter
 * @returns {string}
 */
export function statusMeaning(letter) {
  return XY_MEANING[letter] ?? letter
}
