...
export class GitService extends TypertRemoteService {
  ...
  async generateCommitMessage(cwd, signal) {
    return this.guard(async () => {
      ...
      const system = [
        ...
        '- Write the commit message in Simplified Chinese, including the type prefix translated to Chinese (e.g. 修复: ..., 新增: ..., 重构: ...)',
        '- Keep code identifiers, file names, and technical terms in their original English form',
        '- Return ONLY the commit message, no explanations, no markdown fences, no prefix',
      ].join('\n')

      const userText = [
        'Based on the following staged diff, generate a single Git commit message in Simplified Chinese.',
        '',
        '```diff',
        diffText,
        '```',
   ...
   * @param {string} mode 'ff-only' | 'merge' | 'rebase'
   */
  ...
  /**
   * One-click sync with the branch's upstream: fetch, then compare the local
   * branch against its upstream and reconcile — pull (merge strategy) when
   * behind, push when ahead, pull then push when diverged, no-op when in
   * sync. Reuses the same `git pull --no-rebase` merge semantics as the
   * pull button, so diverged histories merge (auto-merge) instead of
   * refusing. A branch without an upstream (fresh clone) falls back to the
   * first remote's HEAD (the same `push -u` fallback the push button uses).
   * @returns {{ok:true, synced:boolean, pulled:boolean, pushed:boolean, ahead?:number, behind?:number, output:string} | {ok:false,...}}
   */
  async sync(cwd, signal) {
    return this.guard(async () => {
      const root = await this.requireRoot(cwd)

      // 1. fetch so ahead/behind compare against the live remote state
      let fetchOut = ''
      try {
        const fetched = await gitWrite(this.ctx, root, ['fetch', '--all', '--prune'], { signal, timeoutMs: 300_000, remote: true })
        fetchOut = (fetched.stdout + fetched.stderr).trim()
      } catch {
        // offline fetch is not fatal: the compare falls back to last-known refs
      }

      // 2. resolve the upstream we compare / push against
      let upstream = null
      let branch = null
      try {
        branch = (await gitRead(this.ctx, root, ['branch', '--show-current'], { signal })).stdout.trim()
      } catch { branch = '' }
      if (branch === '') {
        return { ok: false, code: GIT_ERROR_CODES.NOT_REPO, message: '当前处于分离 HEAD,无法同步' }
      }
      try {
        upstream = (await gitRead(this.ctx, root, ['rev-parse', '--abbrev-ref', '--symbolic-full-name', `@{u}`], { signal })).stdout.trim()
      } catch {
        upstream = null
      }
      if (upstream === null) {
        // no upstream: mirror push's `-u <remote> <branch>` fallback
        const remote = await this.firstRemote(root, signal)
        if (remote === null) {
          return { ok: false, code: GIT_ERROR_CODES.NO_UPSTREAM, message: '当前分支没有上游,且没有配置远程仓库' }
        }
        upstream = `${remote}/${branch}`
      }

      // 3. measure ahead / behind against the upstream tip
      let ahead = 0
      let behind = 0
      try {
        const { stdout } = await gitRead(this.ctx, root, ['rev-list', '--left-right', '--count', `${upstream}...HEAD`], { signal })
        const m = /^\s*(\d+)\s+(\d+)/.exec(stdout)
        if (m) {
          behind = Number(m[1]) // left side: upstream-only commits
          ahead = Number(m[2])  // right side: local-only commits
        }
      } catch {
        ahead = 0
        behind = 0
      ...
      // 4. reconcile
      const output = [fetchOut]
      let pulled = false
      let pushed = false
      if (behind > 0) {
        const result = await gitWrite(this.ctx, root, ['pull', '--no-rebase'], { signal, timeoutMs: 300_000, remote: true })
        output.push((result.stdout + result.stderr).trim())
        pulled = true
      }
      if (ahead > 0) {
        let pushResult
        try {
          pushResult = await gitWrite(this.ctx, root, ['push'], { signal, timeoutMs: 300_000, remote: true })
        } catch (error) {
          if (!(error instanceof GitError) || error.code !== GIT_ERROR_CODES.NO_UPSTREAM) throw error
          const remote = await this.firstRemote(root, signal)
          const current = await this.currentBranch(root, signal)
          if (remote === null || current === null) throw error
          pushResult = await gitWrite(this.ctx, root, ['push', '-u', remote, current], { signal, timeoutMs: 300_000, remote: true })
        }
        output.push((pushResult.stdout + pushResult.stderr).trim())
        pushed = true
      }
...
markRemote(GitService.prototype, 'sync')
